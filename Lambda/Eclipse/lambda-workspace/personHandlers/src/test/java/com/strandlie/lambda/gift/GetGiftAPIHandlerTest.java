 ...
 */
public class GetGiftAPIHandlerTest {

    private static GiftRequest initialGiftInput;
    private static GiftRequest getGiftInput;
    ...
    private void setupInitialDatabase(Context ctx) {
    	...
    	initialGiftInput.setItemID(itemResponse.getId());
    }
    ...
    @Test
    public void testGetGiftAPIHandler() {
        AddGiftAPIHandler addGiftHandler = new AddGiftAPIHandler();
        GetGiftAPIHandler getGiftHandler = new GetGiftAPIHandler();
        
        Context ctx = createContext();
        setupInitialDatabase(ctx);

        GiftResponse addGiftOutput = (GiftResponse) addGiftHandler.handleRequest(initialGiftInput, ctx);
        getGiftInput.setId(addGiftOutput.getId());
        
        GiftResponse getGiftOutput = (GiftResponse) getGiftHandler.handleRequest(getGiftInput, ctx);
        
        Assert.assertEquals(initialGiftInput.getGiverID(), getGiftOutput.getGiverID());
        Assert.assertEquals(initialGiftInput.getRecepientID(), getGiftOutput.getRecepientID());
        Assert.assertEquals(initialGiftInput.getItemID(), getGiftOutput.getItemID());
        Assert.assertEquals(initialGiftInput.getQuantity(), getGiftOutput.getQuantity());
    }
}
